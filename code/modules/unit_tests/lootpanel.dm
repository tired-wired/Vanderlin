...
/datum/unit_test/lootpanel/contents/Run()
	...
	var/mob/living/carbon/human/labrat = allocate(/mob/living/carbon/human)
	...
	var/obj/item/storage/backpack/bag = allocate(/obj/item/storage/backpack, one_over)
	...
	TEST_ASSERT_EQUAL(searchable.item, bag, "Second item should be the box")
	...
	allocate(/obj/item/storage/backpack, one_over)
	...
	TEST_ASSERT_EQUAL(length(panel.contents), 2, "Contents should repopulate with the new bag")
